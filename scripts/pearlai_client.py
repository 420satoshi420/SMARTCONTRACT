#!/usr/bin/env python3
"""
PearlAI Protocol CLI Client & Automation Toolkit
Interact with PearlAIToken (NEW) and PearlAIPool AMM/Staking Resource.
"""
...
def cmd_status(args):
    ...
    token_addr = dep["contracts"]["PearlAIToken"]["address"]
    pool_addr = dep["contracts"]["PearlAIPool"]["address"]
    ...
    print("=" * 60)
    print("🌐 PEARLAI PROTOCOL LIVE STATUS")
    print("=" * 60)
    ...
    print(f"• PEARL Token Price    : {token_price_eth:.6f} ETH (${token_price_usd:.4f} USD)")
    print(f"• Pool Reserves (ETH): {eth_val:,.4f} ETH")
    print(f"• Pool Reserves (PEARL): {token_val:,.2f} NEW")
...
def cmd_swap_eth(args):
    ...
    print(f"🔄 Swapping {amount_eth} ETH for PEARL tokens...")
    out = run_cast(
...
def cmd_swap_token(args):
    ...
    print(f"🔄 Approving {args.amount} PEARL tokens for pool...")
    run_cast(
        f"send {token_addr} 'approve(address,uint256)' {pool_addr} {amount_token} --private-key {pk} --rpc-url {rpc}"
    )
    print(f"🔄 Swapping {args.amount} PEARL for ETH...")
    out = run_cast(
...
def main():
    parser = argparse.ArgumentParser(description="PearlAI Protocol CLI Interaction Toolkit")
    ...
    # swap-eth
    p_swap_eth = subparsers.add_parser("swap-eth", help="Swap ETH for PEARL tokens")
    ...
    # swap-token
    p_swap_tok = subparsers.add_parser("swap-token", help="Swap PEARL tokens for ETH")
